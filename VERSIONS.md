...
**日期**：2026-09-25
**回滚**：`git checkout pre-v6.1-three-entries -- .`

**核心**：作者指令「全球 top VC 顶级认知」。`STYLE.md` 新增 v6.1「顶级认知四问」：非共识（主流叙事 vs 本篇）、二阶效应（6–18 个月传导链首尾）、资本配置（钱/算力/人从哪挪到哪）、反证（✗ 线不高于现值）。任何一问答不上就不发；对抗审查提示词必含四问。

**同日**：停更 11 天后三连发 ENTRY 137（Meta Muse）、138（TypeSafe Jev）、139（Nscale 招股书 × Anthropic），每篇两路一手核实 + 查漏评论员 + 一位审查员；台账 2026-09-25 对账六条（131/135/133/132/128/134）；修复 JSON-LD ItemList 中 ENTRY 133/134 被历史构建误改的 Day 号；构建脚本通用化为 `buildN.py`（元数据外置、前篇锚点实测、同日多篇可连跑）。
...
**核心**：作者反馈「我看了日记，看不懂，所有日记请重点突出商业价值和投资价值」。全站 130 篇重构：**审计机器移出正文，正文只留商业判断**。

**改动**：
- 正文 500 字 → **200-300 字**（实际 203-348、均值 286），结构固定为 判断 → 事实 → 「对创业者／对投资人」落点 → 金句
- 新增 `entry-foot` 尾注块（虚线分隔、小字）：来源、证伪口、口径注、更正、利益披露、免责声明全部下沉，不计入正文字数
- 修掉一个可读性元凶：来源行原本被塞在第一段中间，读者一上来就撞上几百字引文
- `STYLE.md` 升 v6.0；`daily-prompt.md` 同步；`main.css?v=10`
- llms-full.txt 的 74 个全文节同步为新正文

**审查**：28 agent 流水线（13 批改写 + 13 批对抗审查），审查修正 56 篇；主进程硬性复核字数、空话落点、无中生有。
拼接器修过一个严重 bug：条目在 HTML 里按编号倒序排列，按 n 降序替换会毁掉后续偏移（首次运行毁了 66 篇、已回滚），
改为按文件位置倒序 + 条目数/尾注数完整性断言。
...
**日期**：2026-08-24
**回滚**：`git checkout pre-restyle-500 -- .`

**核心**：全站精炼改版（作者指令）。ENTRY 16-128 中 112 篇正文重写为 ≤500 字：观点第一句、一篇一观点、去 AI 腔。观点、核心数字、证伪口、认错/更正、利益披露全部保留；原长文存 git 历史。

**同步**：
- 新增 `STYLE.md` 风格契约（daily-prompt.md 引用为硬约束）
- llms-full.txt 的 ENTRY 57-128 节替换为新正文
- 标题、日期、entry-number 数字条、entry-img 配图、theses.html 台账不动
