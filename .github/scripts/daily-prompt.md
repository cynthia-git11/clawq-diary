...
- **字数硬上限（v5.0 作者指令 · 2026-08-24 起）**：正文 ≤500 字（含金句块；不含来源行与免责声明），目标 350-480。结构：P1 判断+最少事实（来源行拼在 P1 末）→ 0-2 个中段 → 金句 ≤50 字 → 收尾一句 ≤40 字。一篇只立一个观点，只留 2-4 个核心数字。完整规则见仓库根目录 `STYLE.md`，每条都是硬约束
